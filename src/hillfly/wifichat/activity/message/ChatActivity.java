...
public class ChatActivity extends BaseMessageActivity implements
	...
	public void onClick(View v) {
		...
		case R.id.message_plus_layout_picture:// 发送图片
			ImageUtils.selectPhoto(ChatActivity.this);
			hidePlusBar();
			break;

		case R.id.message_plus_layout_camera:// 打开相机发送
			mCameraImagePath = ImageUtils.takePicture(ChatActivity.this);
			hidePlusBar();
			break;

		case R.id.message_plus_layout_file:// 发送file
			showFileChooser();
			hidePlusBar();
			break;
		}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		switch (requestCode) {
		case ImageUtils.INTENT_REQUEST_CODE_ALBUM:// 选择图片完成
			if (data == null) {
				return;
			}
			if (resultCode == RESULT_OK) {
				...
				if (cursor != null) {
					...
					if (cursor.getCount() > 0 && cursor.moveToFirst()) {
						...
						// else {
						if (path != null) {
							ImageUtils.createThumbnail(this, path, IMAG_PATH
									+ File.separator);
		...
		case FILE_SELECT_CODE: {// 选择文件完成，准备发送
			if (resultCode == RESULT_OK) {
//				Uri uri = data.getData();
//				String path = uri.getPath();
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		case FILE:
			...
			fileIntent.setType("*/*");
			fileIntent.setData(Uri.parse("file://" + msg.getMsgContent()));
