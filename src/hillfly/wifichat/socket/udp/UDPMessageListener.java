...
public class UDPMessageListener implements Runnable {
	...
	public void processMessage(int commandNo, IPMSGProtocol ipmsgRes,
		...
		case IPMSGConst.IPMSG_BR_ENTRY: {
			...
			sendUDPdata(IPMSGConst.IPMSG_ANSENTRY,
					receiveDatagramPacket.getAddress(), mLocalUser);
		...
		case IPMSGConst.IPMSG_SENDMSG: {
			...
			case IMAGE:
				...
				msg.setMsgContent(BaseApplication.IMAG_PATH + File.separator
						...
						+ msg.getMsgContent());
				final String THUMBNAIL_PATH = BaseApplication.THUMBNAIL_PATH
			...
			case FILE:
				...
				tcpService.startReceive();
				sendUDPdata(IPMSGConst.IPMSG_CONFIRM_FILE_DATA, senderIp);// 发送已经开始接受的通知
			...
			if (msg.getContentType() == CONTENT_TYPE.IMAGE
					|| msg.getContentType() == CONTENT_TYPE.VOICE) {// 不是文本，做延迟处理，等文件写到了disk再处理，以免null
				final File file = new File(msg.getMsgContent());
				int k = 0;
				while (!file.exists()) {
					k++;
					if (k > 10) {
			...
			// 加入数据库
			mDBOperate.addChattingInfo(senderIMEI, SessionUtils.getIMEI(),
